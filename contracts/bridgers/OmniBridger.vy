...
@deploy
def __init__():
    assert extcall IERC20(CRV20).approve(OMNI_BRIDGE, max_value(uint256))
...
def bridge(_token: address, _to: address, _amount: uint256):
    ...
    if _token != CRV20 and not self.is_approved[_token]:
        assert extcall IERC20(_token).approve(OMNI_BRIDGE, max_value(uint256))
        self.is_approved[_token] = True

    extcall OmniBridge(OMNI_BRIDGE).relayTokens(_token, _to, _amount)
